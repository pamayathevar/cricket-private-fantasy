...
export function ProductionMatches({ leagueId, memberId, roster, availableFixtureIds, openTeam, openHistory }: { leagueId: string; memberId: string; roster: Player[]; availableFixtureIds: Array<string | undefined>; openTeam: (fixtureId: string) => void; openHistory: (fixtureId: string) => void }) {
  ...
  return <ScrollView contentContainerStyle={x.screen}>
    ...
    {matches.length ? <><View style={x.fixtureFilterHeading}><Text style={x.fixtureFilterHeadingText}>FILTER FIXTURES</Text>{matchesWidth < 520 ? <Text style={x.horizontalScrollHint}>SWIPE →</Text> : null}</View><ScrollView horizontal accessibilityRole="tablist" style={x.fixtureFilterScroller} showsHorizontalScrollIndicator={false} contentContainerStyle={x.fixtureFilters}>{([['ALL', 'All'], ['UPCOMING', 'Upcoming'], ['COMPLETED', 'Completed'], ['PUBLISHED', 'Published']] as Array<[typeof matchFilter, string]>).map(([value, label]) => <TouchableOpacity key={value} accessibilityRole="tab" accessibilityLabel={`${label} fixtures`} accessibilityState={{ selected: matchFilter === value }} style={[x.fixtureFilter, matchFilter === value && x.fixtureFilterActive]} onPress={() => { setMatchFilter(value); setExpanded(""); }}><Text style={[x.fixtureFilterText, matchFilter === value && x.fixtureFilterTextActive]}>{label}</Text></TouchableOpacity>)}</ScrollView></> : null}
    {!matches.length ? <Empty title="No fixtures yet" text="Fixtures will appear after the league schedule is imported." /> : null}
    {matches.length && !visibleMatches.length ? <Empty title="No matching fixtures" text="Choose another fixture filter to see matches." /> : null}
    {visibleMatches.map(match => {
      ...
      return <View key={match.id} style={x.cardBlock}>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Match ${match.match_number}, ${match.home?.code} versus ${match.away?.code}, ${statusLabel}`} accessibilityState={{ expanded: open }} style={x.matchHeader} onPress={() => setExpanded(open ? "" : match.id)}>
          <View style={x.matchNumberBadge}><Text style={[x.matchNumberLabel, x.textMutedAccessible]}>MATCH</Text><Text style={x.matchNumberValue}>{match.match_number}</Text></View>
        ...
        {open ? points.length ? <View>
          ...
          {points.map(point => {
            ...
            return <View key={point.player_id ?? point.player?.full_name} style={x.pointRow}><View style={x.pointPlayer}><View style={x.playerLabelRow}><Text style={x.name}>{point.player?.full_name}</Text>{(specialLabels[match.id]?.[point.player?.full_name] ?? []).map(label => <SpecialPlayerBadge key={label} label={label} />)}</View><View style={x.inlineMeta}><IplTeamBadge code={player?.team} /><OwnerBadge owner={player?.owner} label={player?.owner === "Available" ? "Open player" : player?.owner ?? "—"} compact /></View></View><Text style={x.pointCell}>{fmt(point.batting_points)}</Text><Text style={x.pointCell}>{fmt(point.bowling_points)}</Text><Text style={x.pointCell}>{fmt(point.fielding_points)}</Text><Text style={x.pointCell}>{fmt(point.bonus_points)}</Text>{royaltyMode ? <Text style={[x.pointCell, playerRoyaltyTotal > 0 && x.royaltyColumn]}>{fmt(playerRoyaltyTotal)}</Text> : null}<Text style={x.pointTotal}>{fmt(Number(point.total_points) + (royaltyMode ? playerRoyaltyTotal : 0))}</Text></View>;
          })}
