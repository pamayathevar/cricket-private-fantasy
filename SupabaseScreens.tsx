...
export function ProductionPlayerSquad({ leagueId, canEdit, onAvailabilityChanged }: { leagueId: string; canEdit: boolean; onAvailabilityChanged: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("league_players").select("id,active,acquisition_price,bid_price,owner:league_members(id,display_name),player:players(full_name,role,team:cricket_teams(code))").eq("league_id", leagueId),
        ...
        {!collapsed ? teamPlayers.map(player => {
          ...
          return <View key={playerKey}>
            ...
            {playerOpen && points ? <View style={x.squadPointsBreakdown}>
              ...
              <BreakdownLine label="Total points" value={points.total} strong />
            </View> : null}
          </View>;
        }) : null}
      </View>;
...
export function ProductionHistory({ leagueId }: { leagueId: string }) {
  ...
  useEffect(() => {
    ...
    ]).then(([matchResult, transferResult, periodResult, ownershipResult, ruleResult, formatResult]) => {
      const firstError = matchResult.error ?? transferResult.error ?? periodResult.error ?? ownershipResult.error ?? ruleResult.error ?? formatResult.error;
      if (firstError) { setError(firstError.message); return; }
        ...
        {open ? lineups.length ? lineups.map(lineup => {
          ...
          return <View key={key} style={x.ownerBlock}>
            ...
            {ownerOpen ? (lineup.lineup_players ?? []).sort((a: any, b: any) => a.slot - b.slot).map((entry: any) => {
              ...
              // Match the published scoring RPC and its league-format switch.
              const borrowed = leagueFormat.other_owner_deductions_enabled && auctionOwner !== owner;
...
const x = StyleSheet.create({
  ...
  squadPlayerPoints: { color: "#174D3D", fontSize: 11, fontWeight: "900", marginLeft: 9 }, squadPointsPending: { color: "#8A9691", fontSize: 11, textAlign: "right", fontWeight: "800", marginLeft: 9 }, squadPlayerChevron: { color: "#61756D", fontSize: 9, fontWeight: "900", marginLeft: 7 }, squadPointsBreakdown: { backgroundColor: "#F0F4F1", paddingHorizontal: 28, paddingVertical: 8 }, squadPointsWarning: { backgroundColor: "#FFF0EC", borderRadius: 10, padding: 10, marginBottom: 10 }, squadPointsWarningText: { color: "#7A4036", fontSize: 9, fontWeight: "700" },
