...
export function ProductionSquads({ leagueId, currentOwner, roster, specialSelection }: { leagueId: string; currentOwner: string; roster: Player[]; specialSelection?: SpecialSelectionConfig | null }) {
  ...
  const ownerFiltersApplied = roleFilter !== "ALL" || specialFilter !== "ALL" || playerSort !== "points";
...
{ownerFiltersApplied ? <TouchableOpacity accessibilityRole="button" accessibilityLabel="Reset owner squad filters" style={x.ownerSquadsFilterReset} onPress={() => { setRoleFilter("ALL"); setSpecialFilter("ALL"); setPlayerSort("points"); }}><Text style={x.ownerSquadsFilterResetText}>Reset filters</Text></TouchableOpacity> : null}
</> : null}
</View>
...
<TouchableOpacity accessibilityRole="button" accessibilityLabel={`${owner}’s squad, ${ownerPlayers.length} players, ${fmt(total)} points`} accessibilityState={{ expanded: open }} style={[x.ownerSquadHeader, compact && x.ownerSquadHeaderCompact, open && { backgroundColor: theme.soft }]} onPress={() => setExpanded(open ? "" : owner)}>
