...
export function ProductionHistory({ leagueId, currentOwner, requestedFixtureId = "", requestedScorecardFixtureId = "", scorecardBackRequest = 0, onScorecardStateChange, onCloseRequestedScorecard }: { leagueId: string; currentOwner: string; requestedFixtureId?: string; requestedScorecardFixtureId?: string; scorecardBackRequest?: number; onScorecardStateChange?: (open: boolean) => void; onCloseRequestedScorecard?: () => void }) {
  ...
  useEffect(() => {
    ...
    setMatches([]);
    Promise.all([
      supabase.from("fixtures").select("id,external_ref,match_number,stage,status,scoring_status,lineup_lock_at,scheduled_start,venue,scorecard_source_url,home:cricket_teams!fixtures_home_team_id_fkey(code),away:cricket_teams!fixtures_away_team_id_fkey(code),score_ingestion_batches(status,calculation_version,source_url,review_artifact,published_at),player_match_points(player_id,batting_points,bowling_points,fielding_points,bonus_points,total_points,raw_stats,breakdown,calculation_version,published_at,player:players(id,full_name,role,team:cricket_teams(code))),member_match_scores(lineup_id,total_points,rank,calculation_breakdown),lineup_submissions(id,status,validation_status,captain_player_id,vice_captain_player_id,impact_player_id,impact_type,member:league_members(id,display_name),lineup_players(slot,player:players(id,full_name,role,team:cricket_teams(code))),lineup_boosters(target_player_id,booster:booster_rules(code,player_multiplier,match_multiplier)))").eq("league_id", leagueId).order("match_number", { ascending: false }),
      ...
      supabase.from("lineup_rule_sets").select("version,effective_from_match_number,carry_forward_enabled,captain_multiplier,vice_captain_multiplier,impact_multiplier,other_owner_penalty_percent,other_owner_minimum_penalty").eq("league_id", leagueId).order("effective_from_match_number").order("version"),
      ...
      supabase.from("special_player_score_adjustments").select("fixture_id,player_id,source_member_id,recipient_member_id,adjustment_type,final_player_contribution,rate_percent,minimum_fee,adjustment_points,calculation_breakdown,player:players(full_name,team:cricket_teams(code)),source:league_members!special_player_score_adjustments_source_member_id_fkey(display_name),recipient:league_members!special_player_score_adjustments_recipient_member_id_fkey(display_name)").eq("league_id", leagueId).in("adjustment_type", ["regular_royalty", "marquee_royalty"]),
      supabase.from("league_members").select("id,display_name,role,status").eq("league_id", leagueId).eq("status", "active").in("role", ["league_admin", "owner"]),
    ]).then(([matchResult, transferResult, periodResult, phaseResult, ownershipResult, ruleResult, formatResult, specialRuleResult, boosterResult, royaltyResult, memberResult]) => {
      if (cancelled) return;
      const firstError = matchResult.error ?? transferResult.error ?? periodResult.error ?? phaseResult.error ?? ownershipResult.error ?? ruleResult.error ?? formatResult.error ?? specialRuleResult.error ?? boosterResult.error ?? royaltyResult.error ?? memberResult.error;
      ...
      // A locked fixture reveals every eligible owner's effective XI. Older
      // data can legitimately have no fixture-specific row because the League
      // screen carried the prior XI only in client state. Build those missing
      // teams read-only here so Results reflects the lock contract without
      // mutating history simply because someone opened this screen. The score
      // publication RPC materializes the same rows before scoring.
      const activeOwners = (memberResult.data ?? []).filter((member: any) => member.id && member.display_name);
      const chronologicalMatches: any[] = [];
      [...matchesWithBoosters]
        .sort((left: any, right: any) => Number(left.match_number) - Number(right.match_number))
        .forEach((match: any) => {
          const targetLineups = [...(match.lineup_submissions ?? [])];
          const lockTime = new Date(match.lineup_lock_at ?? match.scheduled_start ?? "").getTime();
          const isLocked = match.status !== "scheduled" || (Number.isFinite(lockTime) && Date.now() >= lockTime);
          const fixtureRule = [...(ruleResult.data ?? [])]
            .filter((rule: any) => Number(rule.effective_from_match_number ?? 1) <= Number(match.match_number))
            .sort((left: any, right: any) => Number(right.effective_from_match_number ?? 1) - Number(left.effective_from_match_number ?? 1) || Number(right.version) - Number(left.version))[0];
          const canCarry = isLocked
            && ["scheduled", "live", "completed"].includes(match.status)
            && !["published", "corrected"].includes(match.scoring_status)
            && fixtureRule?.carry_forward_enabled !== false;

          if (canCarry) {
            activeOwners.forEach((member: any) => {
              const hasLineup = targetLineups.some((lineup: any) => lineup.member?.id === member.id && ["submitted", "locked"].includes(lineup.status));
              if (hasLineup) return;

              const sourceLineup = [...chronologicalMatches]
                .reverse()
                .filter((sourceMatch: any) => !isNoResultFixture(sourceMatch.status))
                .flatMap((sourceMatch: any) => sourceMatch.lineup_submissions ?? [])
                .find((lineup: any) => lineup.member?.id === member.id
                  && ["submitted", "locked"].includes(lineup.status)
                  && lineup.validation_status !== "needs_changes");
              if (!sourceLineup) return;

              targetLineups.push({
                ...sourceLineup,
                id: `auto-carried:${match.id}:${member.id}`,
                status: "locked",
                validation_status: "valid",
                member: { id: member.id, display_name: member.display_name },
                lineup_players: (sourceLineup.lineup_players ?? []).map((entry: any) => ({ ...entry })),
                lineup_boosters: [],
                auto_carried: true,
              });
            });
          ...
          chronologicalMatches.push({ ...match, lineup_submissions: targetLineups });
        });
      const visibleMatches = chronologicalMatches
        .sort((left: any, right: any) => Number(right.match_number) - Number(left.match_number))
        .filter((match: any) => match.status !== "scheduled" || (match.lineup_submissions?.some((lineup: any) => lineup.status === "submitted" || lineup.status === "locked") ?? false));
      ...
      return <View key={match.id} style={[x.resultsMatchCard, open && x.resultsMatchCardOpen, { borderLeftColor: resultTone.accent }]} onLayout={event => {
        ...
        {open ? <><View style={[x.historyVisibility, lineupsRevealed ? x.historyVisibilityRevealed : x.historyVisibilityPrivate]}><View style={[x.historyVisibilityIcon, lineupsRevealed ? x.historyVisibilityIconRevealed : x.historyVisibilityIconPrivate]}><Text style={x.historyVisibilityIconText}>{noResult ? "↺" : lineupsRevealed ? "XI" : "◉"}</Text></View><View style={x.grow}><Text style={x.resultsXiSectionEyebrow}>MATCH {match.match_number} · {match.home?.code} VS {match.away?.code}</Text><Text style={x.historyVisibilityTitle}>{noResultSettled ? "Fixture settled as No Result" : noResult ? "No Result settlement pending" : lineupsRevealed ? "Owner XIs" : "Your XI only"}</Text><Text style={x.historyVisibilityText}>{noResultSettled ? "This XI was cancelled, its usage was returned, later unlocked XIs were reset, and the first later locked XI was recharged against the last valid team." : noResult ? "A league administrator must complete settlement before usage is refunded and later XI charges are corrected." : lineupsRevealed ? `${lineups.length} owner team${lineups.length === 1 ? "" : "s"} · Submitted and auto-carried XIs are shown after lock.` : "Only your submitted XI is visible until this match locks."}</Text></View></View>{lineups.length ? <View style={x.resultsOwnerList}>{lineups.map(lineup => {
          ...
          const scoreBreakdown = score?.calculation_breakdown && typeof score.calculation_breakdown === "object"
            ...
            : {};
          const ownerRoyaltyRows = royaltyAdjustments.filter(row => row.fixture_id === match.id && row.recipient_member_id === lineup.member?.id);
          const ownerRoyaltyFromRows = ownerRoyaltyRows.reduce((total, row) => total + Number(row.adjustment_points ?? 0), 0);
          const ownerRoyaltyTotal = ownerRoyaltyRows.length ? ownerRoyaltyFromRows : Number(scoreBreakdown.royalty_points ?? 0);
          const ownerTotalPoints = Number(score?.total_points ?? 0);
          const ownerMatchPoints = ownerTotalPoints - ownerRoyaltyTotal;
          ...
          return <View key={key} style={[x.resultsOwnerCard, ownerOpen && x.resultsOwnerCardOpen, ownerOpen && { borderColor: isCurrentOwner ? UI_TOKENS.colors.primary : ownerColor.border }]}>
            <TouchableOpacity accessibilityRole="button" accessibilityLabel={`${owner}’s XI for Match ${match.match_number}, ${lineup.lineup_players?.length ?? 0} players, ${score ? `${fmt(ownerMatchPoints)} match points, ${fmt(ownerRoyaltyTotal)} royalty points, ${fmt(ownerTotalPoints)} total points` : "points pending"}`} accessibilityState={{ expanded: ownerOpen }} style={[x.resultsOwnerHeader, compact && x.resultsOwnerHeaderCompact, isCurrentOwner && x.resultsOwnerHeaderCurrent, ownerOpen && x.resultsOwnerHeaderOpen]} onPress={() => { setExpandedOwner(ownerOpen ? "" : key); setExpandedPlayer(""); }}>
              ...
              <View style={[x.resultsOwnerIdentity, compact && x.resultsOwnerIdentityCompact]}><View style={x.ownerNameRow}><Text numberOfLines={1} style={[x.resultsXiName, compact && x.resultsOwnerNameCompact]}>{owner}’s XI</Text>{isCurrentOwner ? <View style={[x.youBadge, { backgroundColor: UI_TOKENS.colors.primary }]}><Text style={x.youBadgeText}>YOU</Text></View> : null}{lineup.auto_carried ? <View style={x.historyAutoCarriedBadge}><Text style={x.historyAutoCarriedBadgeText}>AUTO</Text></View> : null}{boosterCode ? <View style={x.historyBoosterBadge}><Text style={x.historyBoosterBadgeText}>{boosterCode}</Text></View> : null}</View><Text numberOfLines={1} style={[x.resultsXiRowMeta, compact && x.resultsOwnerMetaCompact]}>{lineup.lineup_players?.length ?? 0} players · {lineup.auto_carried ? "Carried from last valid XI · 0 transfers" : `${matchTransfers} match transfer${matchTransfers === 1 ? "" : "s"}`}</Text></View>
              {score ? <View style={[x.resultsOwnerScore, compact && x.resultsOwnerScoreCompact]}><View style={x.resultsOwnerScoreMetric}><Text style={x.resultsOwnerMatchPoints}>{fmt(ownerMatchPoints)}</Text><Text style={x.resultsOwnerScoreLabel}>MATCH PTS</Text></View><View style={x.resultsOwnerScoreMetric}><Text style={x.resultsOwnerRoyalty}>+{fmt(ownerRoyaltyTotal)}</Text><Text style={[x.resultsOwnerScoreLabel, x.resultsOwnerRoyaltyLabel]}>ROY</Text></View><View style={[x.resultsOwnerScoreMetric, x.resultsOwnerTotalMetric]}><Text style={x.resultsOwnerPoints}>{fmt(ownerTotalPoints)}</Text><Text style={x.resultsOwnerPointsLabel}>TOTAL PTS</Text></View></View> : <View style={[x.historyLineupStatus, lineup.auto_carried ? x.historyLineupStatusCarried : lineup.status === "locked" ? x.historyLineupStatusLocked : x.historyLineupStatusSubmitted]}><Text style={x.historyLineupStatusText}>{lineup.auto_carried ? "CARRIED" : lineup.status.toUpperCase()}</Text></View>}<View style={x.resultsOwnerChevron}><Text style={x.resultsOwnerChevronText}>{ownerOpen ? "▲" : "▼"}</Text></View>
            </TouchableOpacity>
            {ownerOpen ? <><View style={[x.resultsXiContext, { borderLeftColor: isCurrentOwner ? UI_TOKENS.colors.primary : ownerColor.accent }]}><View style={[x.resultsXiContextMark, { backgroundColor: ownerColor.soft }]}><Text style={[x.resultsXiContextMarkText, { color: ownerColor.strong }]}>XI</Text></View><View style={x.grow}><Text style={x.resultsXiContextEyebrow}>VIEWING OWNER TEAM</Text><Text style={[x.resultsXiContextTitle, { color: ownerColor.strong }]}>{owner}’s XI</Text><Text style={x.resultsXiContextMeta}>Match {match.match_number} · {match.home?.code} vs {match.away?.code} · {lineup.lineup_players?.length ?? 0} players</Text><View style={x.resultsXiContextStats}><Text style={x.transferUsed}>{matchTransfers} transfer{matchTransfers === 1 ? "" : "s"} this match</Text><Text style={x.transferBalance}>{transferPeriod ? `Period: ${usedThroughMatch} of ${periodLimit} used` : "Transfer period not configured"}</Text></View></View></View>{(lineup.lineup_players ?? []).sort((a: any, b: any) => a.slot - b.slot).map((entry: any) => {
              ...
              const playerOpen = expandedPlayer === playerKey;
              const playerRoyaltyRows = ownerRoyaltyRows.filter(row => row.player_id === player.id);
              const playerRoyaltyTotal = playerRoyaltyRows.reduce((total, row) => total + Number(row.adjustment_points ?? 0), 0);
              return <View key={player.id}><TouchableOpacity accessibilityRole="button" accessibilityLabel={`${player.full_name}, ${player.team?.code ?? "team unavailable"}, ${playerPoints ? `${fmt(contribution)} player points${playerRoyaltyTotal > 0 ? ` and ${fmt(playerRoyaltyTotal)} royalty points` : ""}` : "points pending"}`} accessibilityState={{ expanded: playerOpen }} style={[x.historyPlayer, compact && x.resultsPlayerCompact]} onPress={() => setExpandedPlayer(playerOpen ? "" : playerKey)}><Text style={[x.chevron, compact && x.resultsPlayerChevronCompact]}>{playerOpen ? "▲" : "▼"}</Text><View style={[x.grow, compact && x.resultsPlayerMainCompact, { marginLeft: 7 }]}><View style={x.playerLabelRow}><Text numberOfLines={1} style={[x.playerName, compact && x.resultsPlayerNameCompact]}>{entry.slot}. {player.full_name}</Text>{(specialLabels[match.id]?.[player.full_name] ?? []).map(label => <SpecialPlayerBadge key={label} label={label} />)}{markers.map(marker => <Text key={marker} style={x.marker}>{marker}</Text>)}</View><View style={x.playerMetaRow}><IplTeamBadge code={player.team?.code} /><Text style={x.roleText}>{player.role}</Text><OwnerBadge owner={ownership === "Open" ? "Open player" : ownership === "Mine" ? owner : auctionOwner} label={ownership} compact /><Text style={[x.baseText, playerPoints && Number(playerPoints.total_points) < 0 && x.playerValueNegative]}>{playerPoints ? `Base ${fmt(playerPoints.total_points)}` : "Points pending"}</Text></View></View>{playerPoints ? <View style={[x.resultsPlayerScore, compact && x.resultsPlayerScoreCompact]}>{playerRoyaltyTotal > 0 ? <Text style={x.resultsPlayerRoyalty}>ROY +{fmt(playerRoyaltyTotal)}</Text> : null}<Text style={[x.playerValue, x.resultsPlayerContribution, contribution < 0 && x.playerValueNegative]}>{fmt(contribution)} pts</Text></View> : null}</TouchableOpacity>{playerOpen && playerPoints ? <View style={x.playerBreakdown}><BreakdownLine label="Batting" value={playerPoints.batting_points} /><BreakdownLine label="Bowling" value={playerPoints.bowling_points} /><BreakdownLine label="Fielding" value={playerPoints.fielding_points} /><BreakdownLine label="Bonus" value={playerPoints.bonus_points} /><BreakdownLine label="Base total" value={playerPoints.total_points} strong />{grossContribution !== Number(playerPoints.total_points) ? <BreakdownLine label="After player multipliers" value={grossContribution} /> : null}{ownershipDeduction > 0 ? <BreakdownLine label={uniqueMode ? "Other-player usage fee" : "Ownership deduction"} value={-ownershipDeduction} /> : null}{matchMultiplier !== 1 ? <BreakdownLine label={`After ${booster?.booster?.code} (${matchMultiplier}×)`} value={contribution} /> : null}<BreakdownLine label="Final player contribution" value={contribution} strong />{playerRoyaltyRows.length ? <View style={x.resultsPlayerRoyaltySection}><View style={x.resultsPlayerRoyaltyHeading}><View style={x.grow}><Text style={x.resultsPlayerRoyaltyTitle}>OWNER ROYALTY (ROY)</Text><Text style={x.resultsPlayerRoyaltyHelp}>Credited to {owner} because another owner borrowed this player.</Text></View><Text style={x.resultsPlayerRoyaltyTotal}>+{fmt(playerRoyaltyTotal)} pts</Text></View>{playerRoyaltyRows.map((row, index) => <View key={`${row.source_member_id}:${index}`} style={x.resultsPlayerRoyaltyRow}><View style={x.grow}><Text style={x.resultsPlayerRoyaltyNames}>Borrowed by {row.source?.display_name ?? "Owner"}</Text><Text style={x.resultsPlayerRoyaltyFormula}>{row.adjustment_type === "marquee_royalty" ? "Marquee" : "Regular"} · {fmt(row.rate_percent)}% of {fmt(row.final_player_contribution)}{Number(row.minimum_fee ?? 0) ? ` · minimum ${fmt(row.minimum_fee)}` : ""}</Text></View><Text style={x.resultsPlayerRoyaltyValue}>+{fmt(row.adjustment_points)}</Text></View>)}</View> : null}</View> : null}</View>;
            })}</> : null}
          </View>;
...
function statsFromRaw(rawStats: unknown): PlayerMatchStats | null {
  ...
  return {
    ...
    directWickets: numberValue(raw.directWickets ?? raw.direct_wickets),
...
const x = StyleSheet.create(normalizeUiStyles({
  ...
  resultsOwnerScore: { minWidth: 142, flexShrink: 0, flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 8, marginLeft: 9 },
  resultsOwnerScoreCompact: { minWidth: 126, gap: 5, marginLeft: 5 },
  resultsOwnerScoreMetric: { minWidth: 29, alignItems: "flex-end" },
  resultsOwnerMatchPoints: { color: UI_TOKENS.colors.ink, fontSize: 12, lineHeight: 16, fontWeight: "900" },
  resultsOwnerScoreLabel: { color: UI_TOKENS.colors.muted, fontSize: 6, lineHeight: 8, fontWeight: "900", letterSpacing: 0.35 },
  resultsOwnerRoyalty: { color: "#704091", fontSize: 12, lineHeight: 16, fontWeight: "900" },
  resultsOwnerRoyaltyLabel: { color: "#704091" },
  resultsOwnerTotalMetric: { minWidth: 42, borderLeftWidth: 1, borderLeftColor: UI_TOKENS.colors.border, paddingLeft: 7 },
  ...
  resultsPlayerScore: { minWidth: 62, flexShrink: 0, alignItems: "flex-end", marginHorizontal: 8 },
  resultsPlayerScoreCompact: { minWidth: 58, alignSelf: "flex-start", marginLeft: 6, marginRight: 0, marginTop: 3 },
  resultsPlayerRoyalty: { color: "#704091", fontSize: 8, lineHeight: 11, fontWeight: "900", marginBottom: 2 },
  resultsPlayerContribution: { marginHorizontal: 0 },
  resultsPlayerRoyaltySection: { backgroundColor: "#F8F2FF", borderWidth: 1, borderColor: "#E2D2EF", borderRadius: 10, marginTop: 9, paddingHorizontal: 10, paddingVertical: 7 },
  resultsPlayerRoyaltyHeading: { flexDirection: "row", alignItems: "center", marginBottom: 4 },
  resultsPlayerRoyaltyTitle: { color: "#704091", fontSize: 8, fontWeight: "900", letterSpacing: 0.5 },
  resultsPlayerRoyaltyHelp: { color: "#74667B", fontSize: 7, lineHeight: 11, marginTop: 2 },
  resultsPlayerRoyaltyTotal: { color: "#704091", fontSize: 12, fontWeight: "900", marginLeft: 10 },
  resultsPlayerRoyaltyRow: { minHeight: 34, flexDirection: "row", alignItems: "center", borderTopWidth: 1, borderTopColor: "#E9DFF0", paddingVertical: 5 },
  resultsPlayerRoyaltyNames: { color: "#3D2A49", fontSize: 8, fontWeight: "900" },
  resultsPlayerRoyaltyFormula: { color: "#74667B", fontSize: 7, lineHeight: 10, marginTop: 1 },
  resultsPlayerRoyaltyValue: { color: "#704091", fontSize: 10, fontWeight: "900", marginLeft: 8 },
