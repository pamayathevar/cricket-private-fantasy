  ...
  return <ScrollView ref={historyScrollRef} contentContainerStyle={x.screen}>
    ...
    {filteredMatches.length ? filteredMatches.map(match => {
      ...
      return <View key={match.id} style={x.cardBlock} onLayout={event => {
        ...
        {open ? <><View style={[x.historyVisibility, lineupsRevealed ? x.historyVisibilityRevealed : x.historyVisibilityPrivate]}><View style={[x.historyVisibilityIcon, lineupsRevealed ? x.historyVisibilityIconRevealed : x.historyVisibilityIconPrivate]}><Text style={x.historyVisibilityIconText}>{lineupsRevealed ? "✓" : "◉"}</Text></View><View style={x.grow}><Text style={x.historyVisibilityTitle}>{lineupsRevealed ? "Lineups revealed" : "Pre-lock privacy"}</Text><Text style={x.historyVisibilityText}>{lineupsRevealed ? `${lineups.length} submitted team${lineups.length === 1 ? " is" : "s are"} visible after lineup lock.` : "Only your submitted XI is visible. Other teams appear when the lineup locks."}</Text></View></View>{lineups.length ? lineups.map(lineup => {
          ...
          return <View key={key} style={x.ownerBlock}>
            <TouchableOpacity style={[x.row, { backgroundColor: ownerColor.soft, borderLeftWidth: isCurrentOwner ? 8 : 5, borderLeftColor: ownerColor.accent, borderBottomColor: ownerColor.border }]} onPress={() => setExpandedOwner(ownerOpen ? "" : key)}>
              {score?.rank ? <View style={x.rankSlot}>{medal ? <Text accessibilityLabel={`Rank ${rank}`} style={x.rankMedal}>{medal}</Text> : <Text style={[x.rank, { color: ownerColor.strong }, isCurrentOwner && x.currentRankingRank]}>#{score.rank}</Text>}</View> : null}
              <OwnerAvatar owner={owner} current={isCurrentOwner} /><View style={x.grow}><View style={x.ownerNameRow}><Text style={[x.ownerDisplayName, { color: ownerColor.strong }]}>{owner}</Text>{isCurrentOwner ? <View style={[x.youBadge, { backgroundColor: ownerColor.accent }]}><Text style={x.youBadgeText}>YOU</Text></View> : null}{boosterCode ? <View style={x.historyBoosterBadge}><Text style={x.historyBoosterBadgeText}>{boosterCode}</Text></View> : null}</View><Text style={x.ownerMeta}>{lineup.lineup_players?.length ?? 0} players · {score ? "points published" : "awaiting points"}{boosterCode === "2UP" ? " · match total doubled" : boosterCode === "3X" ? " · Triple Impact used" : boosterCode === "SUP-TR" ? " · Super Transfer used" : ""}</Text><View style={x.transferSummary}><Text style={x.transferUsed}>{matchTransfers} transfer{matchTransfers === 1 ? "" : "s"} this match</Text><Text style={x.transferBalance}>{transferPeriod ? `${usedThroughMatch}/${periodLimit} used · ${balanceAfterMatch} remaining · ${transferPeriod.name}` : "period not configured"}</Text></View></View>
              {score ? <Text style={[x.value, { color: ownerColor.strong }]}>{fmt(score.total_points)} pts</Text> : <View style={[x.historyLineupStatus, lineup.status === "locked" ? x.historyLineupStatusLocked : x.historyLineupStatusSubmitted]}><Text style={[x.historyLineupStatusText, { color: ownerColor.strong }]}>{lineup.status.toUpperCase()}</Text></View>}<Text style={[x.chevron, { color: ownerColor.strong }]}>{ownerOpen ? "▲" : "▼"}</Text>
            </TouchableOpacity>
...
const x = StyleSheet.create({
  ...
  historyVisibility: { flexDirection: "row", alignItems: "center", paddingHorizontal: 13, paddingVertical: 10, borderTopWidth: 1, borderBottomWidth: 1 }, historyVisibilityPrivate: { backgroundColor: "#FFF8E6", borderColor: "#E9D79A" }, historyVisibilityRevealed: { backgroundColor: "#EAF6EE", borderColor: "#B8D9C2" }, historyVisibilityIcon: { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center", marginRight: 9 }, historyVisibilityIconPrivate: { backgroundColor: "#D9A400" }, historyVisibilityIconRevealed: { backgroundColor: "#2D8650" }, historyVisibilityIconText: { color: "#FFFFFF", fontSize: 12, fontWeight: "900" }, historyVisibilityTitle: { color: "#233C34", fontSize: 10, fontWeight: "900" }, historyVisibilityText: { color: "#66776F", fontSize: 8, lineHeight: 12, marginTop: 2 }, historyLineupStatus: { borderRadius: 8, paddingHorizontal: 9, paddingVertical: 5, marginHorizontal: 7 }, historyLineupStatusLocked: { backgroundColor: "rgba(255,255,255,0.55)", borderWidth: 1, borderColor: "rgba(80,95,90,0.18)" }, historyLineupStatusSubmitted: { backgroundColor: "rgba(255,255,255,0.72)", borderWidth: 1, borderColor: "rgba(80,95,90,0.18)" }, historyLineupStatusText: { fontSize: 8, fontWeight: "900", letterSpacing: 0.4 },
