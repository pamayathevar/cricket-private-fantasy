...
function TeamSelection({ ownerName, selected, setSelected, captain, setCaptain, vice, setVice, submitted, setSubmitted, impactPlayer, setImpactPlayer, impactType, setImpactType, boosterCode, setBoosterCode, boosterPlayer, setBoosterPlayer }: { ownerName: string; selected: string[]; setSelected: (players: string[]) => void; captain: string; setCaptain: (name: string) => void; vice: string; setVice: (name: string) => void; submitted: boolean; setSubmitted: (value: boolean) => void; impactPlayer: string; setImpactPlayer: (name: string) => void; impactType: ImpactType; setImpactType: (type: ImpactType) => void; boosterCode: BoosterCode; setBoosterCode: (code: BoosterCode) => void; boosterPlayer: string; setBoosterPlayer: (name: string) => void }) {
  ...
  const transfers = chosen.filter(p => p.owner !== ownerName).length;
  const displayedTransfers = boosterCode === "SUP-TR" ? "Unlimited" : `${transfers}/105`;
  ...
  const optionalMarkerWarnings = [!captain && "Captain not selected. You may still submit without the 2× Captain bonus.", !vice && "Vice-Captain not selected. You may still submit without the 1.5× VC bonus.", !impactPlayer && "BAI/BOI not selected. Impact is optional for this match."].filter(Boolean) as string[];
  const errors = [selected.length !== 11 && `Select exactly 11 players (${selected.length}/11)`, count("BA") < 2 && "At least 2 batters required", count("BO") < 2 && "At least 2 bowlers required", count("WK") < 1 && "At least 1 wicketkeeper required", count("AL") < 1 && "At least 1 all-rounder required", maxTeam > 7 && "Maximum 7 from one IPL team", total > 100 && `₹${(total - 100).toFixed(1)}m over budget`, captain && vice && captain === vice && "Captain and vice-captain must differ", impactPlayer && !impactType && "Choose BAI or BOI for the Impact player", impactPlayer && (impactPlayer === captain || impactPlayer === vice) && "Impact player cannot be captain or vice-captain", boosterCode === "3X" && !boosterPlayer && "Select the player who receives 3X", boosterCode === "3X" && boosterPlayer && !selected.includes(boosterPlayer) && "The 3X player must be in your XI"].filter(Boolean) as string[];
  const toggle = (name: string) => { setSubmitted(false); if (selected.includes(name)) { setSelected(selected.filter(x => x !== name)); if (captain === name) setCaptain(""); if (vice === name) setVice(""); if (impactPlayer === name) { setImpactPlayer(""); setImpactType(""); } if (boosterPlayer === name) setBoosterPlayer(""); } else setSelected([...selected, name]); };
  const resetXI = () => { setSelected([]); setCaptain(""); setVice(""); setImpactPlayer(""); setImpactType(""); setBoosterCode(""); setBoosterPlayer(""); setSubmitted(false); setShowIssues(false); };
  const chooseBooster = (code: BoosterCode) => { const next = boosterCode === code ? "" : code; setBoosterCode(next); if (next !== "3X") setBoosterPlayer(""); setSubmitted(false); };
  const selectFixture = (match: typeof upcomingMatches[number]) => { setActiveMatchId(match.id); setExpandedTeams([match.home, match.away]); setBoosterCode(""); setBoosterPlayer(""); setSubmitted(false); setShowIssues(false); };
  ...
  const renderTeam = (team: string) => {
    ...
    return <View key={team} style={s.teamGroup} onLayout={event => { teamPositions.current[team] = event.nativeEvent.layout.y; }}>
      ...
      {expanded && teamPlayers.map(p => { const active = selected.includes(p.name); const ownership = p.owner === ownerName ? "Mine" : p.owner === "Available" ? "OpenPlayer" : `Owned by ${p.owner}`; const hasImpact = impactPlayer === p.name; const hasLeadership = captain === p.name || vice === p.name; return <View key={p.name} onLayout={event => { playerPositions.current[`${team}:${p.name}`] = event.nativeEvent.layout.y; }} style={[s.playerRow, active && s.playerActive, focusedPlayer === p.name && s.playerFocused]}><TouchableOpacity style={s.playerMain} onPress={() => { setFocusedPlayer(p.name); toggle(p.name); }}><View style={[s.checkbox, active && s.checkboxActive]}><Text style={s.check}>{active ? "✓" : ""}</Text></View><View style={{ flex: 1, marginLeft: 10 }}><Text style={s.playerName}>{p.name}</Text><Text style={s.meta}>{p.role} · {ownership}</Text></View><Text style={s.price}>₹{p.price}m</Text></TouchableOpacity>{active && <View style={s.markers}><Marker text="C" active={captain === p.name} disabled={hasImpact} onPress={() => { setCaptain(captain === p.name ? "" : p.name); setSubmitted(false); }} /><Marker text="VC" active={vice === p.name} disabled={hasImpact} onPress={() => { setVice(vice === p.name ? "" : p.name); setSubmitted(false); }} /><Marker text="BAI" active={impactPlayer === p.name && impactType === "BAI"} disabled={hasLeadership} onPress={() => { const clearing = impactPlayer === p.name && impactType === "BAI"; setImpactPlayer(clearing ? "" : p.name); setImpactType(clearing ? "" : "BAI"); setSubmitted(false); }} /><Marker text="BOI" active={impactPlayer === p.name && impactType === "BOI"} disabled={hasLeadership} onPress={() => { const clearing = impactPlayer === p.name && impactType === "BOI"; setImpactPlayer(clearing ? "" : p.name); setImpactType(clearing ? "" : "BOI"); setSubmitted(false); }} />{boosterCode === "3X" && <Marker text="3X" active={boosterPlayer === p.name} onPress={() => { setBoosterPlayer(boosterPlayer === p.name ? "" : p.name); setSubmitted(false); }} />}</View>}</View>; })}
    </View>;
  };
  return <View style={s.teamScreen}><ScrollView ref={teamScrollRef} contentContainerStyle={s.teamContent}>
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.fixtureStrip} contentContainerStyle={s.fixtureStripContent}>{upcomingMatches.map((match, index) => { const active = match.id === activeMatchId; return <TouchableOpacity key={match.id} style={[s.fixtureCard, active && s.fixtureCardActive]} onPress={() => selectFixture(match)}><Text style={[s.fixtureNumber, active && s.fixtureTextActive]}>MATCH {match.id.replace("M", "")}</Text><Text style={[s.fixtureTeams, active && s.fixtureTextActive]}>{match.home} vs {match.away}</Text><Text style={[s.fixtureTime, active && s.fixtureTextActive]}>{match.day} · {match.time}</Text><Text style={[s.fixtureStatus, submitted && index === 0 ? s.statusSubmitted : null]}>{submitted && index === 0 ? "Submitted" : "XI carried · booster empty"}</Text></TouchableOpacity>; })}</ScrollView>
    ...
    <View style={s.selectionSummary}><Summary label="PLAYERS" value={`${selected.length}/11`} bad={selected.length !== 11} /><Summary label="COST" value={`₹${total.toFixed(1)}m`} bad={total > 100} /><Summary label="TRANSFERS" value={displayedTransfers} bad={false} /></View>
    ...
    <View style={s.impactHelp}><Text style={s.impactHelpTitle}>C, VC and Impact markers are optional</Text><Text style={s.impactHelpText}>BAI/BOI cannot be applied to the same player as C/VC. Conflicting buttons are disabled. Tap an active marker again to remove it.</Text></View>
    {optionalMarkerWarnings.map(warning => <View key={warning} style={s.warningCard}><Text style={s.warningText}>⚠ {warning}</Text></View>)}
    {impactWarnings.map(warning => <View key={warning} style={s.warningCard}><Text style={s.warningText}>⚠ {warning}</Text></View>)}
    <Text style={s.selectedTitle}>Match booster</Text>
    <Text style={s.helper}>Optional · only one booster can be used for this match.</Text>
    <View style={s.boosterGrid}>
      <BoosterCard code="3X" name="Triple Impact" detail="1 remaining · all phases" active={boosterCode === "3X"} onPress={() => chooseBooster("3X")} />
      <BoosterCard code="2UP" name="Double Up" detail="Phase 1 use available" active={boosterCode === "2UP"} onPress={() => chooseBooster("2UP")} />
      <BoosterCard code="SUP-TR" name="Super Transfer" detail="1 remaining · all phases" active={boosterCode === "SUP-TR"} onPress={() => chooseBooster("SUP-TR")} />
    </View>
    ...
    {boosterCode === "2UP" && <View style={s.boosterHelp}><Text style={s.boosterHelpTitle}>Your final match total will be doubled</Text><Text style={s.boosterHelpText}>One 2UP is available in matches 1–35 and one in matches 36–70.</Text></View>}
...
function BoosterCard({ code, name, detail, active, onPress }: { code: Exclude<BoosterCode, "">; name: string; detail: string; active: boolean; onPress: () => void }) { return <TouchableOpacity style={[s.boosterCard, active && s.boosterCardActive]} onPress={onPress}><Text style={[s.boosterCode, active && s.boosterCodeActive]}>{code}</Text><Text style={[s.boosterName, active && s.boosterNameActive]}>{name}</Text><Text style={[s.boosterDetail, active && s.boosterDetailActive]}>{detail}</Text></TouchableOpacity>; }
...
const s = StyleSheet.create({
  ...
  boosterGrid: { flexDirection: "row", gap: 7, marginTop: 8 },
  boosterCard: { flex: 1, minHeight: 91, borderWidth: 1, borderColor: "#D8E0DC", backgroundColor: "white", borderRadius: 12, padding: 10 },
  boosterCardActive: { borderColor: "#174D3D", backgroundColor: "#174D3D" },
  boosterCode: { color: "#174D3D", fontSize: 13, fontWeight: "900" },
  boosterCodeActive: { color: "#DDFB72" },
  boosterName: { color: "#334D44", fontSize: 9, fontWeight: "900", marginTop: 5 },
  boosterNameActive: { color: "white" },
  boosterDetail: { color: "#84918C", fontSize: 7, lineHeight: 11, marginTop: 4 },
