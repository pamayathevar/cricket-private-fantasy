...
## Audited score ingestion

Apply `202608040072_score_ingestion_batches.sql` before deploying the League Admin score-import interface. It creates a league-scoped ingestion ledger, validates compiler artifacts again on the server, makes identical fixture/source retries idempotent, records corrections as new calculation versions, and removes browser access to the lower-level raw-points staging RPC. Staging and publication remain separate admin actions.

Run `verify_migration_072.sql` immediately afterward. Every returned value must be `true`. In staging, also submit the same known review artifact twice and confirm the second response reports `idempotent: true` without creating another calculation version. Follow the operational checklist in `docs/SCORE_INGESTION_RUNBOOK.md`; deploy the database migration before the client.

Apply `202608040073_materialize_locked_carry_forward_lineups.sql` after migration 072. It adds an idempotent, insert-only fixture RPC that creates missing locked lineup rows from each active owner's last eligible valid XI, with zero transfers and no booster. Score publication invokes it before completeness checks; the Results client independently presents the same missing teams read-only as **AUTO / CARRIED** so opening Results never mutates historical data. Existing drafts and real submissions are not overwritten. Run `verify_migration_073.sql`; every returned value must be `true`.

Apply `202608040074_automatic_fixture_lifecycle.sql` after migration 073. It adds a league-scoped, time-guarded reconciliation RPC used by the app at startup and every 30 seconds while a league is open. Once `lineup_lock_at` is reached, it materializes eligible missing carry-forward XIs, changes real submissions to `locked`, and advances only `scheduled -> live`. It never marks a fixture completed; score publication or the explicit No Result workflow remains responsible for the final status. The RPC requires active membership, cannot advance another league, is idempotent under concurrent clients, and writes an audit event for each transition. Run `verify_migration_074.sql`; every boolean must be `true`. After an active member opens each league, its final diagnostic query should return no overdue scheduled fixtures.

Roll out migration 074 before the client that calls it, then open each active league and verify both the diagnostic query and the `fixture_advanced_at_lock` audit event. To stop future automatic transitions, roll the client back first and revoke/drop `reconcile_due_fixture_lifecycle`; do not reverse already locked lineups or `live` fixtures automatically. If a bad lock timestamp advanced a fixture incorrectly, correct that fixture through an audited administrator operation after confirming no scoring or later-lineup dependency has used it.

Apply `20260817221734_score_ingestion_jobs.sql` before enabling URL-based score import. It creates an admin-readable, service-managed job ledger and exposes only the request RPC to authenticated users. The RPC independently verifies fixture scope, league-admin authority, fixture state and an HTTPS source URL. Browser clients cannot insert, update or delete jobs. Run `verify_migration_075.sql`; every boolean must be `true`.

Then deploy the authenticated `ingest-score-source` Edge Function. ESPNcricinfo (including its legacy `cricinfo.com` domain) and Cricbuzz, including their subdomains, are present in its built-in source-host allowlist; use the optional `SCORE_SOURCE_ALLOWED_HOSTS` backend secret only for additional approved providers. Configure `SCORE_INGESTION_ADAPTER_URL` and `SCORE_INGESTION_ADAPTER_TOKEN` as Supabase backend secrets before expecting normal provider web pages to be parsed. The function prepares a review artifact only; staging and publishing remain separate admin actions. See `docs/SCORE_INGESTION_RUNBOOK.md` for the security model, provider requirements, test matrix and rollout order.
