-- Make transfer charging follow the configured acquisition mode.
-- All-open leagues charge every player introduced since the prior submitted XI.
-- Auction leagues retain the existing rule: only introduced players not owned by
-- the submitting member consume transfers. The configured first match of every
-- transfer period remains free in both modes.
begin;
