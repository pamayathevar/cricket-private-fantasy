-- One authoritative, fixture-scoped source for Unique/Marquee UI indicators.
begin;
...
begin
  ...
  -- SQL Editor/service calls have no auth.uid(). App JWT calls must belong to the league.
  if auth.uid() is not null and not public.is_league_member(v_fixture.league_id) then raise exception 'Active league membership required'; end if;
