...
- In **Rules → Match Scoring**, administrators can configure the ESPNcricinfo and Cricbuzz series pages once. **Discover & save fixture URLs** matches only the exact match number and both teams, records the mapping in the audit log, and prefills future score imports. Per-match URL entry remains available when a provider has not listed a fixture yet.
